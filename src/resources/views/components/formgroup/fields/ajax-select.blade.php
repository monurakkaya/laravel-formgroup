...
>
    ...
    @foreach ($options['models'] as $option)
        ...
        <option @if(old($name, request()->get($name, '')) == $option->{(($options['value'] ?? 'id'))}) selected @endif value="{{ $option->{($options['value'] ?? 'id')} }}">{{ $option->{($options['label'] ?? 'name')} }}</option>
    @endforeach
...
@push('js')
    <script>
        $(function () {
                ...
                .on('change', function () {
                    ...
                    $.get(
                        ...
                        function (response) {
                            ...
                            @if (old($options['child']['name'], request()->get($options['child']['name'], false)) !== false)
                            $('select[name={{$options['child']['name']}}]').val({{ old($options['child']['name'], request()->get($options['child']['name'])) }});
                            @endif
                            child_holder.show();
                        }
                    )
            ...
            @if (old($name, request()->get($name, false)) !== false)
